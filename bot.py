...
class GoogleSheetsFinanceTracker:
    ...
    def get_credentials(self):
        """Get credentials from environment variable or file with proper error handling"""
        ...
        if credentials_json:
            ...
            try:
                ...
                logger.error(
                    "💡 Make sure it's a valid JSON string without extra quotes")
                raise
        else:
            logger.info(
                "ℹ️ GOOGLE_CREDENTIALS_JSON not found, checking for credentials.json file")
            # Try to use credentials file (for local development)
            if os.path.exists('credentials.json'):
                logger.info("✅ Found credentials.json file")
                return Credentials.from_service_account_file('credentials.json')
            else:
                logger.error(
                    "❌ No credentials found! Please set GOOGLE_CREDENTIALS_JSON environment variable or add credentials.json file")
                raise FileNotFoundError("No Google credentials found")

    def setup_sheets(self):
        """Setup Google Sheets connection with detailed error reporting"""
        try:
            # Get credentials
            creds = self.get_credentials()

            # ✅ USE THE CORRECT SCOPE THAT WAS WORKING
            scope = [
